...
def choose_finish_line(x, y, M, xmax, ymax):
    ...
    if horizontal:  # move the x coordinate away from the corner
        if fin2x < xmax / 2:
            fin2x = round(fin2x + xmax / 4)
        else:
            fin2x = round(fin2x - xmax / 4)
    else:  # move the y coordinate away from the corner
        if fin2y < ymax / 2:
            fin2y = round(fin2y + ymax / 4)
        else:
            fin2y = round(fin2y - ymax / 4)
    ...
    for y in range(ymax - 1, -1, -1):
        for x in range(xmax):
            if M[x, y]:
                ...
                print('@', end=' ')
            elif y1 == y2 and y == y1 and min(x1, x2) <= x <= max(x1, x2):
                print('-', end=' ')
            elif x1 == x2 and x == x1 and min(y1, y2) <= y <= max(y1, y2):
                print('|', end=' ')
            else:
                print(' ', end=' ')
        print('')
...
# main(n=20, size=24, display=0, title='')
